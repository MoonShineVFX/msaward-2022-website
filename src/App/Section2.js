...
function Section2({data}) {
  ...
  return (
    <div name="top10" className='w-9/12 text-white mx-auto flex flex-col justify-center items-center  relative mt-64 xs:w-full xs:mt-16'>
        ...
        <div className='w-3/5 text-xs text-center text-zinc-400 my-6 xs:w-4/5'>由夢想動畫評審團評選出的優選作品</div>
        <div className='w-5/6 grid grid-cols-6 gap-6 xs:grid-cols-3'>
          {
            data.map((item,index)=>{
              ...
              return(
                <div 
                  key={index}
                  className='bg-black  w-[9vw] aspect-[3/4.3] bg-cover bg-no-repeat bg-center relative hover:-translate-y-3 transition cursor-pointer duration-300 mr-5 mb-10 rounded-md xs:w-[25vw] brightness-75 hover:brightness-105'
